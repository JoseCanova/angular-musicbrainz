...
export class ClassConfigComponent implements OnInit , AfterViewInit {
  ...
  public processMetaClass(key : any , value:any):any{
	  let mapping :any;
	  console.log('the key ' + key + ' the value ' + value);
	  mapping =  this.restService.getMappings()
			   	.pipe (map((x)=>x))
			  	.subscribe(
			  	mappings =>{
  ...
  public processSample(map:Mapping):any{
	  this.restService.processSampleUrl(map.sampleUrl)
	  .subscribe((sample:any)=> map.setSample(sample));
  }
